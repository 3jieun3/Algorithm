...
def combination_rep(i, n, cnt, m, arr):        # (4) n개 중에서 중복 있게 m개 고른 조합 = nHm ?
	if cnt == m:
		save_combi_rep_elements(arr, combi_rep)
		return
	...
	for j in range(i, n):
		combination_rep(j, n, cnt + 1, m, arr + [numbers[j]])
...
combi_rep = []
cnt_rep = [0] * M
combination_rep(0, N, 0, M, [])
print_result(combi_rep)